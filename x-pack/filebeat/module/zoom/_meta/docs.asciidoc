...
This is configured with the `secret.header`, `secret.value` and `crc.provider` settings as shown below.

In addition, Zoom requires webhook validation based on CRC (Challenge-Response Check) for all created or modified webhooks. The `crc.provider` setting should be added to properly handle CRC requests according to the webhook provider's guidelines.
